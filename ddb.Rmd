...
The transformation of the data from POPUM to IDS has been quite straight-forward. This does not mean that the available information is without problems, but it has simplified much of the transfer of data into IDS format. The sources sometimes contain missing data, which can be a problem when it comes to analysis. The missing data due to death records not being preserved as mentioned above are a problem. In family reconstitution, children born are often considered alive if there is no information on them having died and the family is still present in the studied area. When death records are missing we have no reliable information on their death or survival. The Swedish system of catechetical registers reported however the complete population, and therefore we still have the possibility to analyse mortality in most age groups. This is unfortunately not the case when it comes to infant mortality. In many parishes, newborn children were not entered into these registers immediately after birth. The time elapsed between actual birth and it being entered in the register varied in time and between parishes. In the case of Skellefteå for the period when death records are missing, a reasonable assumption is that children with only a birth entry and no further information while the rest of the family is still resident in the parish died quite young. To finally establish this require however further controls and the use of precise definitions, something that we have refrained from in this article. This circumstance is however a good illustration of the need to carefully evaluate the sources and what implications possible deficiencies may have on the analysis.

The context information on linking individuals to their households causes some problems. Families and households are presented together in the catechetical registers. We find the family on the same page in the registers with an empty line separating it from the next family. Households are however difficult to identify. Servants and others are usually presented at the bottom of the page, and if there are several families/households on the same page it is not always obvious to which family they belong. Furthermore, the separation of families/households can also be difficult, particularly in urban environments where migrations were frequent making the registers more difficult to interpret. Making rules for defining households turned out to be very complicated. The information therefore primarily represent the nuclear families as households.
...
analysis of the
same problem. It may be called it *sensitivity analysis* [@co84,chapter 6].  
