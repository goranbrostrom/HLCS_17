...
This table needs to be collapsed before an analysis. Inspired by previous results, we do it as in Table \@ref(tab:sos2).

```{r sos2}
gen1$gmD <- cut(gen1$Mm_deaths, c(-1, 0, 1, 8), 
                labels = c("gmD = 0", "gmD = 1", "gmD = 2+"))
gen1$mD <- cut(gen1$m_deaths, c(-1, 0, 8), 
               labels = c("mD = 0", "mD = 1+"))
x <- with(gen1, table(mD, gmD))

knitr::kable(x, caption = "Contingency table of mother's (mD) and grandmother's (gmD) infant deaths, collapsed.",
             col.names = colnames(x), row.names = TRUE)
```
...
A standard $\chi^2$ test of Table \@ref(tab:sos2) results in a *p*-value of `r round(rr$p.value, 2)`, not very spectacular. 


## Survival analysis, male mortality

In an attemt to match the result in the previous section, the survival analysis is restricted to male mortality and one mother per grandmother.

```{r extsurv, fig.cap = "Cumulative hazards of male mortality by number of grandmother male deaths.", fig.height = 4.2}
ef$childBirthdate <- ef$mBirthdate + ef$enter
eff <- ef[!duplicated(ef$GrandmotherID), ]
fit.c2 <- coxreg(Surv(exit - enter, event) ~ strata(gmDeaths) + gmBirths + Child_birthyear + parity, data = eff[eff$sex == "Male", ])
plot(fit.c2, lty = c(1, 2, 4), 
